...
public class GameManager : MonoBehaviour
{
    ...
    void SpawnPlayerByLevel()
    {
        ...
        GameObject player = Instantiate(playerToSpawn, new Vector3(0f, -4.2f, 0f), Quaternion.identity);

        // Call SpawnShield on the newly spawned player
        PlayerController controller = player.GetComponent<PlayerController>();
        if (controller != null)
        {
            controller.SpawnShield();
    }
    }
